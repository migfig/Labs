...
namespace RelatedRecords.Tests
{
    ...
    public class ConfigurationTests
    {
        ...
        {            
            ...
            Assert.AreEqual(3, _config.Dataset.First().Table.Count);
            Assert.AreEqual("Tickets", _config.Dataset.First().Table.First().name);
            Assert.AreEqual(5, _config.Dataset.First().Table.First().Column.Count);
            ...
            Assert.AreEqual(2, _config.Dataset.First().Relationship.Count);
        ...
        public void Table_To_Select_Query_String_Test()
        {
            ...
            Assert.AreEqual("SELECT [Id],[TicketNumber],[Title],[StatusCodeId],[PriorityId] FROM Tickets",
                query.Trim());
        }
        ...
        public void Column_To_Where_Query_String_Test()
        {
            ...
            query = _config.Dataset.First().Table.First().Column.Last()
                .ToWhereString(new SqlParameter { ParameterName = "PriorityId", Value = 100 },
                    false, ">=");
            Assert.AreEqual("And PriorityId >= 100", query.Trim());
        }
        ...
        public void Build_Related_Tables_Queries()
        {
            ...
            Assert.AreEqual(2, queries.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries).Count());
            Assert.AreEqual("SELECT * FROM TicketsStatusCodes WHERE StatusCodeId = 1;SELECT * FROM TicketsPriorities WHERE PriorityId = 1;", queries);
        }
        ...
        public void Build_Table_Schema()
        {
            ...
            var query = _config.Dataset.First().Table.First()
                .ToSchemaString();
            Assert.AreEqual("create Table [dbo].[Tickets] ([Id] UNIQUEIDENTIFIER NOT NULL PRIMARY KEY,[TicketNumber] BIGINT NOT NULL,[Title] VARCHAR(250) NOT NULL,[StatusCodeId] INT NOT NULL,[PriorityId] INT NOT NULL);", query);
        }
        ...
        public void Build_Table_Sample_Data()
        {
            ...
            var query = _config.Dataset.First().Table.First()
                .ToInsertString();
            Assert.AreEqual("Insert Into [dbo].[Tickets] ([Id],[TicketNumber],[Title],[StatusCodeId],[PriorityId]) Values ('00100000-0000-0000-0000-000000000000',1,'Tickets 1',1,1);", 
                query);
        }
